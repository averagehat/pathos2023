...
class ParameterizedReadFile : public ReadFile{
 ...
 public:
  ...
  ParameterizedReadFile(ReadFile* readFile, ParamsMapping* pm, long initialCycle);
  ParameterizedReadFile(ReadFile* readFile, ParamsMapping* pm, long initialCycle, long numCycles);
  ParameterizedReadFile(ReadFile* readFile, ParamsMapping* pm, long initialCycle, long cyclesOn, long cyclesSkipped);
  ~ParameterizedReadFile();

  bool useThisCycle(long cycleNum);
  ParamsMapping* getMapParams(); // returns a copy
  // returns the same as class function below, but prevents the need for recasting
  void splitFile(set<ParameterizedReadFile*>* putFilesHere, long numFiles);
