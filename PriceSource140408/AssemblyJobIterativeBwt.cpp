...
void AssemblyJobIterativeBwt::constructorHelper(set<ScoredSeq*>* inputSeqs){
  _minOverlap = _alsf->getMinOverlap();
...
void AssemblyJobIterativeBwt::runJob(AssemblyJob::AssemblyThreadedness threadedness){
  if (! _wasRun){
    ...
    if (_gapped){
      if (! _usingMaxOverlap){ seqCollectionBwt = new ScoredSeqCollectionBwt(&_inputSeqs,_alsf); }
      else { seqCollectionBwt = new ScoredSeqCollectionBwt(&_inputSeqs,_alsf,_maxOverlap); }
    } else {
      if (! _usingMaxOverlap){ seqCollectionBwt = new ScoredSeqCollectionBwt(&_inputSeqs,_alsf->getFractId(),_minOverlap); }
      else { seqCollectionBwt = new ScoredSeqCollectionBwt(&_inputSeqs,_alsf->getFractId(),_minOverlap,_maxOverlap); }
    }
