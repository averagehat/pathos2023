...
void Read::gatherSeq(char* collector, char sense){
  #pragma omp critical (Read)
  {
    fillBufferPrivate();
    _buffer->gatherSeq(collector,sense);
  }
}
void Read::gatherScores(float* collector, char sense){
  #pragma omp critical (Read)
  {
    fillBufferPrivate();
    _buffer->gatherScores(collector,sense);
  }
}
void Read::gatherLinks(float* collector, char sense){
  #pragma omp critical (Read)
  {
    fillBufferPrivate();
    _buffer->gatherLinks(collector,sense);
  }
}

void Read::gatherSubseq(char* collector, long position, long length, char sense){
  #pragma omp critical (Read)
  {
    fillBufferPrivate();
    _buffer->gatherSubseq(collector,position,length,sense);
  }
}
void Read::gatherSubScores(float* collector, long position, long length, char sense){
  #pragma omp critical (Read)
  {
    fillBufferPrivate();
    _buffer->gatherSubScores(collector,position,length,sense);
  }
}
void Read::gatherSubLinks(float* collector, long position, long length, char sense){
  #pragma omp critical (Read)
  {
    fillBufferPrivate();
    _buffer->gatherSubLinks(collector,position,length,sense);
  }
}
...
// these are identical to the two functions above
void Read::bottomBuffer(){
  #pragma omp critical (Read)
  {
    bufferPrivate();
  }
}
void Read::deepUnbuffer(){
  #pragma omp critical (Read)
  {
    unbufferPrivate();
  }
...
ScoredSeq * Read::shallowCopy(){
  ...
  {
    fillBufferPrivate();
    seqCopy = new ScoredSeqShallow(_buffer, '+');//copyShallowSeq(_buffer, '+');
  }
  return seqCopy;
