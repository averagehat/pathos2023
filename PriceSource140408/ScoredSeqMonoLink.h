...
#ifndef SCOREDSEQMONOLINK_H
#define SCOREDSEQMONOLINK_H

#include "ScoredSeq.h"
#include "AssemblyException.h"


class ScoredSeqMonoLink : public ScoredSeq {
 public:
  ScoredSeqMonoLink(string seq, vector<float>* scores); // sets all linkages to 1
  ScoredSeqMonoLink(string seq, vector<float>* scores, float link);
  // REQUIRES that len(scores)==len(seq) AND len(links)==len(seq)-1
  ScoredSeqMonoLink(string seq, float* scores); // sets all linkages to 1
  ScoredSeqMonoLink(string seq, float* scores, float link);
  // REPLACES strings with char* arrays
  ScoredSeqMonoLink(char* seq, float* scores, long size);
  ScoredSeqMonoLink(char* seq, float* scores, float link, long size);
  // ALLOWS rep to be exposed (i.e. arrays stored are the actual ones input)
  ScoredSeqMonoLink(bool expRep, char* seq, float* scores, float link, long size);
 ...
 protected:
  ...
  void addPairedEnd(ScoredSeqMonoLink * ss);


 private:
  ScoredSeqMonoLink(ScoredSeqMonoLink* seq, char sense);  //copy constructor
  ...
  void makeDead();
  void replaceDeadPe(ScoredSeqMonoLink * deadReplacement);
