...
void AssemblyJobGraph::makeShallow(bool shallowDelete){
  ...
  // SECURITY CHECKS COMMENTED OUT, SHOULD BE ABLE TO BE BROUGHT BACK
...
void AssemblyJobGraph::collapseOneNode(long tipIndex, long edgeIndex, set<ScoredSeq*>* localProducts,
  ...
  DyProAlignerFactory* dpaf = new DyProAlignerFactory(pmf->calculateMinFractId(&justForParams),
						      pmo->calculateMinOverlap(justForParams.size()),
						      asMatrix);
  delete asMatrix;
  ScoredSeqCollectionBwt* anchorSeqCollection = new ScoredSeqCollectionBwt(&anchorSet, dpaf);
  // don't delete dpaf yet, it will be used to construct the contig collections below
  delete pmf;
  delete pmo;
  ...
  for (int senseN = 0; senseN < 2; ++senseN){
    ...
    set<ScoredSeq*> cbiFound;
    set<ScoredSeq*>::iterator fIt = cbiFound.begin();
    set<ScoredSeq*> cbiNotFoundYet;
    set<ScoredSeq*>::iterator nfyIt = cbiNotFoundYet.begin();
    for (long n = 0; n < numQueries; ++n){
      if ( cbiKeepBool[n] ){ fIt = cbiFound.insert(fIt, cbiArray[n]); }
      else { nfyIt = cbiNotFoundYet.insert(nfyIt, cbiArray[n]); }
    }
    delete [] cbiLengths;
    delete [] cbiKeepBool;
    ...
    if (cbiNotFoundYet.size() > 0){

      ScoredSeqCollectionBwt* cbiNotFoundBwt = new ScoredSeqCollectionBwt(&cbiNotFoundYet, dpaf);

      set<ScoredSeq*> cbiNewFoundContigs;
      MatchSeqTestNotInSet* contigFoundTest = new MatchSeqTestNotInSet();

      vector<Alignment*> matchCollector;
      set<ScoredSeq*>::iterator anchIt = anchorSet.begin();
      while(cbiNewFoundContigs.size() < cbiNotFoundYet.size() and anchIt != anchorSet.end()){
	// find the matches
	ScoredSeq* anchSeq = *anchIt;
	++anchIt;

	cbiNotFoundBwt->getMatches(&matchCollector, anchSeq, sense, contigFoundTest, anchSeq->size(),
				    ScoredSeqCollectionBwt::softMinOvl);
	...
	for (vector<Alignment*>::iterator alIt = matchCollector.begin(); alIt != matchCollector.end(); ++alIt){
	  ...
	  cbiNewFoundContigs.insert(foundSeq);
	  contigFoundTest->addToSet(foundSeq);
	  delete *alIt;
	}
	matchCollector.clear();
      }
      delete cbiNotFoundBwt;
      delete contigFoundTest;

      cbiFound.insert(cbiNewFoundContigs.begin(), cbiNewFoundContigs.end());
    }

    // cbiFound has all the contigs to keep...
    edgeAssembled[edgeIndex].insert(cbiFound.begin(), cbiFound.end());

    // but I need to figure out which ones are to be discarded
    fIt = cbiFound.begin();
    set<ScoredSeq*>::iterator fItEnd = cbiFound.end();
    set<ScoredSeq*> addToDiscard;
    set<ScoredSeq*>::iterator atdIt = addToDiscard.begin();
    for (long n = 0; n < numQueries; ++n){
      if (fIt == fItEnd || (*fIt) != cbiArray[n]){
        atdIt = addToDiscard.insert(atdIt, cbiArray[n]);
      } else { ++fIt; }
    } 
