...
char* ScoredSeqNormalized::getSeq(char sense) { return _innerSeq->getSeq(sense); }
...
void ScoredSeqNormalized::gatherSeq(char* collector, char sense) { _innerSeq->gatherSeq(collector,sense); }
void ScoredSeqNormalized::gatherScores(float* collector, char sense) {
  _innerSeq->gatherScores(collector,sense);
  if (_normCount > 1){
    long size = _innerSeq->size();
    for (long n = 0; n < size; ++n){ collector[n] /= _normCount; }
  }
}
void ScoredSeqNormalized::gatherLinks(float* collector, char sense) {
  _innerSeq->gatherLinks(collector,sense);
  if (_normCount > 1){
    long sizeM1 = _innerSeq->size() - 1;
    for (long n = 0; n < sizeM1; ++n){ collector[n] /= _normCount; }
  }
}

void ScoredSeqNormalized::gatherSubseq(char* collector, long position, long length, char sense){
  _innerSeq->gatherSubseq(collector, position, length, sense);
}
void ScoredSeqNormalized::gatherSubScores(float* collector, long position, long length, char sense){
  _innerSeq->gatherSubScores(collector, position, length, sense);
  if (_normCount > 1){
    for (long n = 0; n < length; ++n){ collector[n] /= _normCount; }
  }
}
void ScoredSeqNormalized::gatherSubLinks(float* collector, long position, long length, char sense){
...
ScoredSeq * ScoredSeqNormalized::shallowCopy(){
  //OK();
  if (_normCount > 1){ return ScoredSeq::copyShallowSeq(this, '+'); }
  else { return _innerSeq->shallowCopy(); }
}
ScoredSeq * ScoredSeqNormalized::flipCopy(){
  //OK();
  if (_normCount > 1){ return ScoredSeq::copyShallowSeq(this, '-'); }
  else { return _innerSeq->flipCopy(); }
}
...
void ScoredSeqNormalized::replaceNested(ScoredSeq* seq){ _innerSeq = seq; }
