...
void ScoredSeqSubseq::gatherSeq(char* collector, char sense) {
  switch (sense){
  case '+': _innerSeq->gatherSubseq(collector, _offset, _size, sense); break;
  case '-': _innerSeq->gatherSubseq(collector, _minusOffset, _size, sense); break;
  default: throw AssemblyException::ArgError("SSSubseq::getSeq, sense must be (+) or (-)");
  }
}
void ScoredSeqSubseq::gatherScores(float* collector, char sense) {
  switch (sense){
  case '+': _innerSeq->gatherSubScores(collector, _offset, _size, sense); break;
  case '-': _innerSeq->gatherSubScores(collector, _minusOffset, _size, sense); break;
  default: throw AssemblyException::ArgError("sense must be '+' or '-'");
  }
}
void ScoredSeqSubseq::gatherLinks(float* collector, char sense) {
  switch (sense){
  case '+': _innerSeq->gatherSubLinks(collector, _offset, _size - 1, sense); break;
  case '-': _innerSeq->gatherSubLinks(collector, _minusOffset, _size - 1, sense); break;
  default: throw AssemblyException::ArgError("sense must be '+' or '-'");
  }
}


void ScoredSeqSubseq::gatherSubseq(char* collector, long position, long length, char sense){
  switch (sense){
  case '+': _innerSeq->gatherSubseq(collector, position + _offset, length, sense); break;
  case '-': _innerSeq->gatherSubseq(collector, position + _minusOffset, length, sense); break;
  default: throw AssemblyException::ArgError("SSSubseq::getSubseq, sense must be (+) or (-)");
  }
}
void ScoredSeqSubseq::gatherSubScores(float* collector, long position, long length, char sense){
  switch (sense){
  case '+': _innerSeq->gatherSubScores(collector, position + _offset, length, sense); break;
  case '-': _innerSeq->gatherSubScores(collector, position + _minusOffset, length, sense); break;
  default: throw AssemblyException::ArgError("SSSubseq::getSubScores, sense must be (+) or (-)");
  }
}
void ScoredSeqSubseq::gatherSubLinks(float* collector, long position, long length, char sense){
  switch (sense){
  case '+': _innerSeq->gatherSubLinks(collector, position + _offset, length, sense); break;
  case '-': _innerSeq->gatherSubLinks(collector, position + _minusOffset, length, sense); break;
  default: throw AssemblyException::ArgError("SSSubseq::getSubLinks, sense must be (+) or (-)");
  }
}
...
void ScoredSeqSubseq::replaceNested(ScoredSeq* seq){ _innerSeq = seq; }
