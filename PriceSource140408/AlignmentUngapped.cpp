...
long AlignmentUngapped::score(AlignmentScoreMatrix* scoreMatrix, bool penalizeTerminalGaps){
  ...
  char* seqSegA = _seqA->getSubseq(_startA, length, '+');
  char* seqSegB = _seqB->getSubseq(_startA - _offset, length, _orientation);
  for (long n = 0; n < length; ++n){
    if (! scoreMatrix->isMatch(seqSegA[n], seqSegB[n])){ ++misCount; }
  }
  delete [] seqSegA;
  delete [] seqSegB;
...
long AlignmentUngapped::scoreOverhangA(AlignmentScoreMatrix* scoreMatrix){
  ...
  long scoreLen = _endA - _startA;

  char* seqSegA = _seqA->getSubseq(_startA, scoreLen, '+');
  char* seqSegB = _seqB->getSubseq(_startA - _offset, scoreLen, _orientation);
  for (long n = 0; n < scoreLen; ++n){
    if (! scoreMatrix->isMatch(seqSegA[n], seqSegB[n])){ ++misCount; }
  }
  delete [] seqSegA;
  delete [] seqSegB;
