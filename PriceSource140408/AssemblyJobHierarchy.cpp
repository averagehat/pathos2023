...
void AssemblyJobHierarchy::runJobHelpIterativeBwt(AssemblyJob::GapStatus gapStatus, AssemblyJob::AssemblyThreadedness threadedness){
  ...
  while ( doRound ){
    ...
    // create the job
    AssemblyJob* asj;
    DyProAlignerFactory * astf = new DyProAlignerFactory(minFractId,minOverlap,_sm);

    if (! _usingMaxOverlap){ asj = new AssemblyJobIterativeBwt(&_seqCarrier, _strandedness, gapStatus, astf, _assemblyMode); }
    else { asj = new AssemblyJobIterativeBwt(&_seqCarrier, _strandedness, gapStatus, astf, _maxOverlap, _assemblyMode); }
    delete astf;
...
void AssemblyJobHierarchy::runJob(AssemblyJob::AssemblyThreadedness threadedness){
  if ( _inputSeqs.empty() ){ _wasRun = true; }
  else if (! _wasRun){
