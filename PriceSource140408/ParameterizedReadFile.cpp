...
ParameterizedReadFile::ParameterizedReadFile(ReadFile* readFile, ParamsMapping* pm, long initialCycle) :
...
ParameterizedReadFile::ParameterizedReadFile(ReadFile* readFile, ParamsMapping* pm, long initialCycle, long numCycles) :
  _initialCycle(initialCycle),
  _isFinite(true),
...
ParameterizedReadFile::ParameterizedReadFile(ReadFile* readFile, ParamsMapping* pm, long initialCycle, long cyclesOn, long cyclesSkipped) :
  _initialCycle(initialCycle),
  _isFinite(false),
  _totalCycles(0),
  _isCyclic(true),
...
void ParameterizedReadFile::splitFile(set<ReadFile*>* putFilesHere, long numFiles){
...
void ParameterizedReadFile::splitFile(set<ParameterizedReadFile*>* putFilesHere, long numFiles){
  set<ReadFile*> splitFiles;
  _readFile->splitFile(&splitFiles, numFiles);
  for (set<ReadFile*>::iterator it = splitFiles.begin(); it != splitFiles.end(); ++it){
    putFilesHere->insert( new ParameterizedReadFile( this, *it ) );
...
bool ParameterizedReadFile::useThisCycle(long cycleNum){
...
void ParameterizedReadFile::openWithCarriers(long numSets){ _readFile->openWithCarriers(numSets); }
void ParameterizedReadFile::openWithCarriers(long numSets, bool getBothEnds){ _readFile->openWithCarriers(numSets,getBothEnds); }
...
void ParameterizedReadFile::openNormWithCarriers(long numSets){ _readFile->openNormWithCarriers(numSets); }
void ParameterizedReadFile::openNormWithCarriers(long numSets, bool getBothEnds){ _readFile->openNormWithCarriers(numSets,getBothEnds); }
