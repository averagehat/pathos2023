...
AssemblyJobNearPerfect::WindowStartAndLenCarrier::WindowStartAndLenCarrier(long seqLen, long maxMisNum){
  ...
  long minNumWindows;
  if (seqLen < baseCase){
    // windows must be at least 1nt long each
    if (seqLen < 2){ minNumWindows = 1; }
    else { minNumWindows = 2; }
  }
...
void AssemblyJobNearPerfect::getLegitMatchesHelper(ScoredSeq* inputSeq, char sense,
  ...
  // this array will be used repeatedly in this loop, but always for local values
  ...
  for (set<ScoredSeqNormalized*>::iterator candIt = allCandidateMatches.begin(); candIt != allCandidateMatches.end(); ++candIt){
    ...
    // avoid repeatedly creating this - here it is with maximum size
    char* compareSeq = new char[ candSeq->size() + 1 ];
