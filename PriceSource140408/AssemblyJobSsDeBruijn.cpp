...
void AssemblyJobSsDeBruijn::testTreeConstructionHelper(set<ScoredSeq*>* foundKmers, SeqNode* node, int stepsToBottom){
  if (stepsToBottom == 0){
    ...
    char kseq[ _kmerSize + 1 ];
    int locKsize = dbNode->getKmerWithSize(&kseq[0], 0);
    kseq[locKsize] = '\0';
    foundKmers->insert( new ScoredSeqMonoScore(&kseq[0], dbNode->getKmerScore(), locKsize) );
  } else {
    char nucList[] = {'A','T','C','G'};
    int stepsToBottomM1 = stepsToBottom - 1;
    for (int n = 0; n < 4; ++n){
      SeqNode* nextNode = node->getBranch( nucList[n] );
      if (nextNode != 0){ testTreeConstructionHelper( foundKmers, nextNode, stepsToBottomM1 ); }
    }
  }
}
...
ScoredSeq* AssemblyJobSsDeBruijn::makeSeqFromPath(DeBruijnNode* node5p, DeBruijnNode* node3p, long numNodes){
  long seqLen = _kmerSizeM1 + numNodes;
  ...
  for (int n = 0; n < _kmerSizeM1; ++n){
    scores[n] = firstScore;
    links[n] = firstScore;
  }
  scores[_kmerSizeM1] = firstScore;

  // fill in the rest
  DeBruijnNode* currentNode = node5p;
  DeBruijnNode* priorNode = 0;
  long pos5p = 0;
  long pos3p = _kmerSizeM1;
  while (currentNode != node3p){
    priorNode = currentNode;
    currentNode = currentNode->best3pLink();
    // fill in values
    //links[pos3p] = priorNode->get3pLinkScore( currentNode->get3pNuc() );
    links[pos3p] = priorNode->get3pLinkScore( currentNode->get3pNuc() );
    if (links[pos5p] < currentNode->get5pLinkScore( cSeq[pos5p] )){
      links[pos5p] = currentNode->get5pLinkScore( cSeq[pos5p] );
    }
    // increment
    pos5p++;
    pos3p++;
    ...
    float currentKmerScore = currentNode->getKmerScore();
    for (long pos = pos5p; pos < pos3p; ++pos){
      if (scores[pos] < currentKmerScore){ scores[pos] = currentKmerScore; }
    }
  }
...
pair<AssemblyJobSsDeBruijn::NucNode*,long> AssemblyJobSsDeBruijn::makeGraphTree(set<ScoredSeq*>* seqs, int kmerSize){
  ...
  if (kmerSize > 0){
    ...
    for (set<ScoredSeq*>::iterator seqIt = seqs->begin(); seqIt != seqs->end(); ++seqIt){
      if ( (*seqIt)->size() > maxSeqSize ){ maxSeqSize = (*seqIt)->size(); }
    }
    char* nucs = new char[maxSeqSize+1];
    float* scores = new float[maxSeqSize+1];
    float* links = new float[maxSeqSize+1];


    int kmerSizeM1 = kmerSize - 1;
    for (set<ScoredSeq*>::iterator seqIt = seqs->begin(); seqIt != seqs->end(); ++seqIt){
      ScoredSeq* seq = *seqIt;
      ...
      SeqNode* currentNodes[kmerSizeM1];
      float worstScores[kmerSizeM1];
      ...
      seq->gatherSeq(nucs,'+');
      seq->gatherScores(scores,'+');
      seq->gatherLinks(links,'+');
	...
	if (nuc == 'N'){
	  ...
	  // i am duplicating the contents of this loop because the activity is different at n==0,
	  // so i avoid the if/else
	  int n = maxCurrentNodeIndex;
	  while (n > 0){
	    --n;
	    SeqNode* oldNode = currentNodes[n];
	    float worstScore = worstScores[n];
	    if ( nucScore < worstScore ){ worstScore = nucScore; }
	    ...
	    currentNodes[n+1] = newNode;
	    worstScores[n+1] = worstScore;
	  }
	  { // n==0
	    SeqNode* oldNode = topNode;
	    float worstScore = nucScore;
	    if ( linkScore < worstScore ){ worstScore = linkScore; }
	    SeqNode* newNode = oldNode->getBranch(nuc);
	    if (newNode == 0){
	      newNode = new NucNode(oldNode,nuc);
	    ...
	    currentNodes[0] = newNode;
	    worstScores[0] = worstScore;
	  }
...
int AssemblyJobSsDeBruijn::SeqNode::getKmerWithSize(char* seqToFillIn, long startCoord){
  // first, figure out how big the kmer is
  int kmerSize = 0;
  SeqNode* currentNode = this;
  while (currentNode != 0 and currentNode->get3pNuc() != '\0'){
...
inline int AssemblyJobSsDeBruijn::NucNode::getNucIndex(char nuc){
  switch ( nuc ){
  case 'A': return 0;
  case 'C': return 1;
  case 'G': return 2;
...
inline int AssemblyJobSsDeBruijn::DeBruijnNode::getNucIndex(char nuc){
  switch ( nuc ){
  case 'A': return 0;
  case 'C': return 1;
  case 'G': return 2;
