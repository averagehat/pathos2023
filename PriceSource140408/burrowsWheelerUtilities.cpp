...
void burrowsWheelerUtilities::bwTransform(long* text, long* sortedSuffixes, long textLen, long* columnL){
  long textLenM1 = textLen - 1;
  for (long n = 0; n < textLen; ++n){
    // note: suffixStart = sortedSuffixes[n];
    if (sortedSuffixes[n] == 0){ columnL[n] = text[textLenM1]; }
...
long* burrowsWheelerUtilities::fillBwtHelperArrays(long* columnL, long textLen, long alphaSize, long* occCounts){
  ...
  long* tableCp1 = &tableC[1]; // optimization to avoid adding 1 to indexes
  for (long c = 0; c < alphaSizeP1; ++c){ tableC[c] = 0; }
  for (long n = 0; n < textLen; ++n){
    tableCp1[columnL[n]]++;
    occCounts[n] = tableCp1[columnL[n]];
  }
  for (long c = 0; c < alphaSize; ++c){ tableCp1[c] += tableC[c]; }
...
// oldIndexSize is the size of the oldIndex array, not necessarily the number of elemetns
//    that is recorded in the zero-th position (array must be oldIndexSize+1 or greater)
// MODIFIES oldIndexes by maybe deleting it, although it may also return it; return value
//    will be oldIndexes if it is not deleted, a new array if it is deleted
long* burrowsWheelerUtilities::bwtCountAndFind(long* query, long qStart, long qSize,
					       ...
					       long* oldIndexes, long oldIndexSize){
  ...
  while ( sp <= ep and i >= qStart ){
    ...
    sp--; // this is an optimization assuming --x takes less time than x - y
    sp = cCounts[c] + findOccCount(sp, c, columnL, textSize, occCounts);
  ...
  if (count < 0){ count = 0; }
  long* indexes;
  if (oldIndexSize < count){
    delete [] oldIndexes;
    indexes = new long[count+1]; // first index is the count; remaining indexes are the indexes of the matches
  } else {
    indexes = oldIndexes;
  }
  indexes[0] = count;

  // this is just to accelerate the loop
  long* indexesOs = &indexes[1];
  long* sortSufOs = &sortedSuffixes[sp];
  for (long n = 0; n < count; ++n){ indexesOs[n] = sortSufOs[n]; }
