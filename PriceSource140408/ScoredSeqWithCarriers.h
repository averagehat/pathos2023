...
class ScoredSeqWithCarriers : public ScoredSeqNested, public ScoredSeqPaired {
 public:
  ...
  ScoredSeq* getNested();
  void replaceNested(ScoredSeq* seq);
  void deepDelete();
  void deepUnbuffer();
 ...
 private:
  ...
  // private constructor for creating PE copy during deletion - 
  // takes in the already-created set array instead of making a new one
  ScoredSeqWithCarriers(ScoredSeq* innerSeq, int numOfSets, set<ScoredSeq*>* carriedSeqSets, ScoredSeqWithCarriers* pe);
