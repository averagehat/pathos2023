...
ScoredSeqWithCarriers::ScoredSeqWithCarriers(ScoredSeq* innerSeq, int numOfSets) :
  ...
  _innerSeq(innerSeq){
  _pe = 0;
  _carriedSeqSets = new set<ScoredSeq*>[ numOfSets+1 ];
  _okToDeletePe = true;
  //_readCount++;
}

ScoredSeqWithCarriers::ScoredSeqWithCarriers(ScoredSeq* innerSeq, int numOfSets, set<ScoredSeq*>* carriedSeqSets, ScoredSeqWithCarriers* pe) :
  _thisIsAlive( true ),
  _numOfSets(numOfSets),
  _carriedSeqSets(carriedSeqSets),
  _pe(pe),
  _innerSeq(innerSeq){
  _okToDeletePe = true;
  //_readCount++;
...
ScoredSeqWithCarriers::~ScoredSeqWithCarriers(){
  ...
  if ( _pe != 0 and _pe->isAlive() ) {
    ...
    ScoredSeqWithCarriers * copyOfThis = new ScoredSeqWithCarriers( _innerSeq, _numOfSets, _carriedSeqSets, _pe );
    copyOfThis->makeDead();
...
void ScoredSeqWithCarriers::gatherSeq(char* collector, char sense) { _innerSeq->gatherSeq(collector, sense); }
void ScoredSeqWithCarriers::gatherScores(float* collector, char sense){ _innerSeq->gatherScores(collector, sense); }
void ScoredSeqWithCarriers::gatherLinks(float* collector, char sense){ _innerSeq->gatherLinks(collector, sense); }

void ScoredSeqWithCarriers::gatherSubseq(char* collector, long position, long length, char sense){
  _innerSeq->gatherSubseq(collector, position, length, sense);
}
void ScoredSeqWithCarriers::gatherSubScores(float* collector, long position, long length, char sense){
  _innerSeq->gatherSubScores(collector, position, length, sense);
}
void ScoredSeqWithCarriers::gatherSubLinks(float* collector, long position, long length, char sense){
  _innerSeq->gatherSubLinks(collector, position, length, sense);
}
...
void ScoredSeqWithCarriers::replaceNested(ScoredSeq* seq){ _innerSeq = seq; }
...
void ScoredSeqWithCarriers::clearCarriedSeqs(int setNum){
  ...
  //delete _carriedSeqSets[setNum];
  //_carriedSeqSets[setNum] = new set<ScoredSeq*>;
}
