...
void ExtendCycle::mapReads(MapReadSetupHelper* mrsh, vector<ScoredSeqWithCarriers*>** hitReadsByFile,
  ...
  if (mrsh->numFilesActive() == 0){
	...
	for (set<int>::iterator it = fileNumsToRemove.begin(); it != fileNumsToRemove.end(); ++it){ thisRoundFileNums.erase( *it ); }
      ...
      // will be used to create re-usable arrays to reduce new/delete calls
      long maxNumPairsPerThread = 0;
      for (int threadN = 0; threadN < numThreadsUsed; ++threadN){
	if (numPairs[threadN] > maxNumPairsPerThread){ maxNumPairsPerThread = numPairs[threadN]; }
      }
      long fullPairsMemoryP1 = maxNumPairsPerThread * numThreadsUsed + 1;

      // TEST BLOCK 1
      // make copies of the reads - this will allow all of the critical blocks to be
      // encountered at the same time and with optimal efficiency - but save the original
      // nested sequences
      ScoredSeq*** readsToMapInner = new ScoredSeq**[ numThreadsP1 ];
      ScoredSeq*** pairsToMapInner = new ScoredSeq**[ numThreadsP1 ];
      // do not thread this because of critical locks on the Read copy method
      for (int threadN = 0; threadN < numThreadsUsed; ++threadN){
	readsToMapInner[threadN] = new ScoredSeq*[numPairs[threadN]+1];
	pairsToMapInner[threadN] = new ScoredSeq*[numPairs[threadN]+1];
	for (long pN = 0; pN < numPairs[threadN]; ++pN){
	  readsToMapInner[threadN][pN] = readsToMap[threadN][pN]->getNested();
	  readsToMap[threadN][pN]->replaceNested( readsToMapInner[threadN][pN]->shallowCopy() );
	  pairsToMapInner[threadN][pN] = pairsToMap[threadN][pN]->getNested();
	  pairsToMap[threadN][pN]->replaceNested( pairsToMapInner[threadN][pN]->shallowCopy() );
	  readsToMapInner[threadN][pN]->deepUnbuffer();
	  pairsToMapInner[threadN][pN]->deepUnbuffer();
	}
      }
      ...
      bool* eitherHasMatchMemory = new bool[ fullPairsMemoryP1 ];

      // do the mapping, and decide what needs to be deleted, in a threaded block
      long* numReadsFiltered = new long[numThreadsUsed+1];

      // reduces calls to "new" in thread-safe manner
      bool* shouldReadsMapMemory = new bool[ fullPairsMemoryP1 ];
      bool* shouldPairsMapMemory = new bool[ fullPairsMemoryP1 ];
      bool* readHasMatchMemory = new bool[ fullPairsMemoryP1 ];
      bool* pairHasMatchMemory = new bool[ fullPairsMemoryP1 ];

      #pragma omp parallel for schedule(dynamic)
      for (int threadN = 0; threadN < numThreadsUsed; ++threadN){
	long memStart = threadN * maxNumPairsPerThread;
	bool* shouldReadsMap = &shouldReadsMapMemory[memStart];
	bool* shouldPairsMap = &shouldPairsMapMemory[memStart];
	...
	bool* readHasMatch = &readHasMatchMemory[memStart];
	bool* pairHasMatch = &pairHasMatchMemory[memStart];
	//eitherHasMatch[threadN] = new bool[ numPairs[threadN]+1 ];
	eitherHasMatch[threadN] = &eitherHasMatchMemory[memStart];
	...
	for (long n = 0; n < numPairs[ threadN ]; ++n){
      ...
      delete [] readHasMatchMemory;
      delete [] pairHasMatchMemory;
      delete [] shouldReadsMapMemory;
      delete [] shouldPairsMapMemory;
	...
	++ufCount;
      ...
      // TEST BLOCK 2
      // replace the nested reads and delete the storage data structure
      #pragma omp parallel for schedule(dynamic)
      for (int threadN = 0; threadN < numThreadsUsed; ++threadN){
	for (long pN = 0; pN < numPairs[threadN]; ++pN){
	  ScoredSeq* innerCopy;
	  innerCopy = readsToMap[threadN][pN]->getNested();
	  readsToMap[threadN][pN]->replaceNested( readsToMapInner[threadN][pN] );
	  innerCopy->deepDelete();
	  innerCopy = pairsToMap[threadN][pN]->getNested();
	  pairsToMap[threadN][pN]->replaceNested( pairsToMapInner[threadN][pN] );
	  innerCopy->deepDelete();
	}
	delete [] readsToMapInner[threadN];
	delete [] pairsToMapInner[threadN];
      }
      delete [] readsToMapInner;
      delete [] pairsToMapInner;

      // this is ok to parallelize because there is no critical lock on read deletion
      #pragma omp parallel for schedule(dynamic)
