...
void ScoredSeqFlipPlus::gatherSeq(char* collector, char sense) { _innerSeq->gatherSeq(collector,sense); }
void ScoredSeqFlipPlus::gatherScores(float* collector, char sense) { _innerSeq->gatherScores(collector,sense); }
void ScoredSeqFlipPlus::gatherLinks(float* collector, char sense) { _innerSeq->gatherLinks(collector,sense); }

void ScoredSeqFlipPlus::gatherSubseq(char* collector, long position, long length, char sense){ _innerSeq->gatherSubseq(collector,position,length,sense); }
void ScoredSeqFlipPlus::gatherSubScores(float* collector, long position, long length, char sense){ _innerSeq->gatherSubScores(collector,position,length,sense); }
void ScoredSeqFlipPlus::gatherSubLinks(float* collector, long position, long length, char sense){ _innerSeq->gatherSubLinks(collector,position,length,sense); }
...
void ScoredSeqFlipPlus::replaceNested(ScoredSeq* seq){ _innerSeq = seq; }
...
void ScoredSeqFlipMinus::gatherSeq(char* collector, char sense) {   //OK();
  switch (sense){
  case '+': _innerSeq->gatherSeq(collector,'-'); break;
  case '-': _innerSeq->gatherSeq(collector,'+'); break;
  default: throw AssemblyException::ArgError("SSFM: bad sense");
  }
}
void ScoredSeqFlipMinus::gatherScores(float* collector, char sense) {   //OK();
  switch (sense){
  case '+': _innerSeq->gatherScores(collector,'-'); break;
  case '-': _innerSeq->gatherScores(collector,'+'); break;
  default: throw AssemblyException::ArgError("SSFM: bad sense");
  }
}
void ScoredSeqFlipMinus::gatherLinks(float* collector, char sense) {   //OK();
  switch (sense){
  case '+': _innerSeq->gatherLinks(collector,'-'); break;
  case '-': _innerSeq->gatherLinks(collector,'+'); break;
  default: throw AssemblyException::ArgError("SSFM: bad sense");
  }
}

void ScoredSeqFlipMinus::gatherSubseq(char* collector, long position, long length, char sense){
  switch (sense){
  case '+': _innerSeq->gatherSubseq(collector,position,length,'-'); break;
  case '-': _innerSeq->gatherSubseq(collector,position,length,'+'); break;
  default: throw AssemblyException::ArgError("SSFM: bad sense");
  }
}
void ScoredSeqFlipMinus::gatherSubScores(float* collector, long position, long length, char sense){
  switch (sense){
  case '+': _innerSeq->gatherSubScores(collector,position,length,'-'); break;
  case '-': _innerSeq->gatherSubScores(collector,position,length,'+'); break;
  default: throw AssemblyException::ArgError("SSFM: bad sense");
  }
}
void ScoredSeqFlipMinus::gatherSubLinks(float* collector, long position, long length, char sense){
  switch (sense){
  case '+': _innerSeq->gatherSubLinks(collector,position,length,'-'); break;
  case '-': _innerSeq->gatherSubLinks(collector,position,length,'+'); break;
  default: throw AssemblyException::ArgError("SSFM: bad sense");
  }
}
...
ScoredSeq * ScoredSeqFlipMinus::shallowCopy(){
  //return ScoredSeq::copyShallowSeq(_innerSeq, '-');
  return _innerSeq->flipCopy();
}
ScoredSeq * ScoredSeqFlipMinus::flipCopy(){
  //return ScoredSeq::copyShallowSeq(_innerSeq, '+');
  return _innerSeq->shallowCopy();
}
bool ScoredSeqFlipMinus::isNested(){ return true; }
ScoredSeq* ScoredSeqFlipMinus::getNested(){ return _innerSeq; }
void ScoredSeqFlipMinus::replaceNested(ScoredSeq* seq){ _innerSeq = seq; }
